...
class LineTrajectory:
    ...
    def publish_trajectory(self, duration=0.0, offset=1.0):
        should_publish = len(self.points) > 1
        if self.visualize and self.traj_pub.get_subscription_count() > 0:
            ...
            if should_publish:
                ...
                marker.scale.x = 0.3
                marker.color.r = 1.0*offset
                marker.color.g = 1.0*offset
                marker.color.b = 1.0
                marker.color.a = 1.0
                for p in self.points:
                    pt = Point()
